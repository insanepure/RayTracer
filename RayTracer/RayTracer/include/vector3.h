...
struct Vector3
{
    ...
    */
    ...
    /*!
    * @brief Calculates the Reflection of the specified Vector3<T> instances.
    *
    * @param vec1 The first vector.
    * @param vec2 The second vector.
    *
    * @returns The reflection of the specified Vector3<T> instances.
    */
    inline static Vector3<T> Reflection(const Vector3<T>& vec1, const Vector3<T>& vec2)
    {
        return Vector3<T>(vec1 - vec2 * Vector3<T>::Dot(vec1,vec2) * 2);
    }
